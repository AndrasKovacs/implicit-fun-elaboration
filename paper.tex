...
$\lambda$-s are not annotated), and infer types for the bodies. Note the eager
insertion in $\einsert\,(\einfer{t}{\Theta_1}{\Gamma[\theta_1],\,x:A})$. This
causes, somewhat unexpectedly, the following example to fail:
\begin{alignat*}{3}
  & \slet\,f : \_ = \lambda\,A\,\{B\}.\,A\,\sin\,\\
  & \slet\,g : \U \to \{B : \U\}\to U = f\,\sin\,...
\end{alignat*}
The issue here is that we immediately apply the elaborated $\lambda\{B\}.\,A$
expression to a fresh meta, which causes the implicit function type to disappear
from the result, and preventing types to unify on the following line. We could
repair this by blocking insertion for implicit $\lambda$-s, but we choose to
keep the algorithm as simple as possible, and in any case overeager insertion
can be fixed by using type annotations to stay in checking mode.
...
\begin{itemize}
  ...
  \item A theory has the \emph{canonicity} property, if every closed term is
    definitionally equal to a canonical term. For example, every closed term
    with a natural number type is a numeral, every $\mathsf{Bool}$ is $\true$ or
    $\false$ and so on. A canonical theory is useful as a \emph{total
      programming language}; if canonicity fails, then some programs compute
    to stuck terms which cannot be viewed as values. In non-total languages,
    progress and type preservation constitute a weaker analogue of canonicity.
...
that of smaller or more conventional theories. In the simplest case, new
features are definable from old features. Here, we can either assume that new
features are merely definitional shorthands, or give an evident injective
translation from the new theory to the old one. Both imply that the desired
properties transfer from the old theory to the new one.
...
\mi{Int} \to A \to A$ with $\mi{Int} \to \{A : \U\}\to A \to A$.
...
$\slet\,x : \_ = \mi{single}\,\mi{id}\,\sin\,$
$\mi{cons}\,\{\mi{IdTy}\}\,x\,\mi{nil}$ works in FCIF, MLF and HML but does not
work in GI, QL and HMF. MLF and HML work here by immediately giving a principal
type to $\mi{single}\,\mi{id}$ which involves generalization with a subtyping bound. In
contrast, FCIF makes no promises about principal typing --- which is not
feasible with dependent types --- and does no generalization, but it can still
infer a type for $\mi{id}$ which can be later constrained to $\mi{IdTy}$.
...
Armed with this knowledge, it inserts a fresh meta in $(f\,x)$. The following
fails though:
\[
    \lambda\,f\,x.\,\mi{pair}\,(f\,x)\,(f\,\{x\}\,x)
\]
Now, FCIF decides upon elaborating $(f\,x)$ that $f$ is a function whose first
...
applications. We could try inserting an unknown curried application after the
first $f$, which could be later refined. We sketch an extension of elaboration
which could possibly handle this example. We extend insertion with
the case which inserts a new curried application whenever we have a meta-headed
type. We omit metacontexts and metasubstitutions for brevity.
\begin{alignat*}{3}
...
%% let e2 = λ f (x : Int). pair (f {id {Int} x} x) (f x) in
