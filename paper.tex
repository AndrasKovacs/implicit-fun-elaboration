...
orthogonal improvements of the presented system.
...
first $f$, which could be later refined. Below we sketch an extension of
$\einsert$ which accomplishes this. We omit metacontexts and metasubstitutions
for brevity.
\begin{alignat*}{3}
...
rise to unification problems where metas are applied to such unknown telescopes.
...
supports this feature is MLF, and recall that MLF's inference algorithms are
already very complex, and MLF does not support dependent types.

We nevertheless sketch a solution, which is not yet fleshed out, but could be
developed in future work. We work with the following example:
\[
...
$f\,(\lambda\,\{A\}.\,x\,\{\alpha_2\,x\,A : \ol{\alpha_0}\})$ for the same
...
$(\lambda\,x\,A.\,(x :: A :: []))$. On the one hand, this rules out solutions
which contain arbitrary terms or non-linear variable occurrences, which would
yield unsolvable patterns. On the other hand, we probably do not throw away many
useful solutions, because all curried applications arise from elaboration, and
we do not have to deal with any of them coming from the surface language. In
short, we try to keep the invariant that metas returning in record types are
only ever solved with order-preserving embeddings.
